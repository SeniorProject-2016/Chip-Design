...
void ADC_SAR_theACLK_Stop(void) 
{
    ...
    ADC_SAR_theACLK_CLKEN &= (uint8)(~ADC_SAR_theACLK_CLKEN_MASK);
	ADC_SAR_theACLK_CLKSTBY &= (uint8)(~ADC_SAR_theACLK_CLKSTBY_MASK);
}
...
{
    if ((ADC_SAR_theACLK_CLKEN & ADC_SAR_theACLK_CLKEN_MASK) != 0u)
    {
    ...
    if (clkDivider != oldDivider)
    {
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                ADC_SAR_theACLK_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((ADC_SAR_theACLK_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
{
    ADC_SAR_theACLK_MOD_SRC |= modeBitMask & (uint8)ADC_SAR_theACLK_MODE_MASK;
}
...
{
    ADC_SAR_theACLK_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(ADC_SAR_theACLK_MODE_MASK));
}
...
{
    return ADC_SAR_theACLK_MOD_SRC & (uint8)(ADC_SAR_theACLK_MODE_MASK);
}
...
void ADC_SAR_theACLK_SetSourceRegister(uint8 clkSource) 
{
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        ADC_SAR_theACLK_MOD_SRC =
            (ADC_SAR_theACLK_MOD_SRC & (uint8)(~ADC_SAR_theACLK_SRC_SEL_MSK)) | clkSource;
    }
