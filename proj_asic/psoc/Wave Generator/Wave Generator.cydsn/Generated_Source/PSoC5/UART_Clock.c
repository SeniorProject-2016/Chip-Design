...
void UART_Clock_Start(void) 
{
    ...
    UART_Clock_CLKEN |= UART_Clock_CLKEN_MASK;
	UART_Clock_CLKSTBY |= UART_Clock_CLKSTBY_MASK;
}
...
void UART_Clock_Stop(void) 
{
    ...
    UART_Clock_CLKEN &= (uint8)(~UART_Clock_CLKEN_MASK);
	UART_Clock_CLKSTBY &= (uint8)(~UART_Clock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: UART_Clock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((UART_Clock_CLKEN & UART_Clock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = UART_Clock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = UART_Clock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* UART_Clock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(UART_Clock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(UART_Clock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: UART_Clock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: UART_Clock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = UART_Clock_GetSourceRegister();
    uint16 oldDivider = UART_Clock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = UART_Clock_CLKEN & UART_Clock_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((UART_Clock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
{
    return CY_GET_REG16(UART_Clock_DIV_PTR);
}


/*******************************************************************************
* Function Name: UART_Clock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    UART_Clock_MOD_SRC |= modeBitMask & (uint8)UART_Clock_MODE_MASK;
}


/*******************************************************************************
* Function Name: UART_Clock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    UART_Clock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(UART_Clock_MODE_MASK));
}


/*******************************************************************************
* Function Name: UART_Clock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return UART_Clock_MOD_SRC & (uint8)(UART_Clock_MODE_MASK);
}


/*******************************************************************************
* Function Name: UART_Clock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = UART_Clock_GetDividerRegister();
    uint8 oldSrc = UART_Clock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        UART_Clock_MOD_SRC |= CYCLK_SSS;
        UART_Clock_MOD_SRC =
            (UART_Clock_MOD_SRC & (uint8)(~UART_Clock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: UART_Clock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return UART_Clock_MOD_SRC & UART_Clock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: UART_Clock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    UART_Clock_PHASE = clkPhase & UART_Clock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: UART_Clock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return UART_Clock_PHASE & UART_Clock_PHASE_MASK;
}
